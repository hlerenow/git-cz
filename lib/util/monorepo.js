...
const isMonorepo = (state) => {
  let isLerna = fs.existsSync(path.join(state.root, 'lerna.json'));
  let isPnpmWorkspace = fs.existsSync(path.join(state.root, 'pnpm-workspace.yaml'));
  return isLerna || isPnpmWorkspace;
};
...
const getAllPackages = (state) => {
  try {
    const dirs = getPackageDirectories(state).map((dir) =>
      path.join(state.root, dir)
    );
